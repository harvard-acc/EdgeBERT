...
SC_MODULE(AuxAxi)
{
 ...
 public: 
  ...
  class MasterTrig: public nvhls_message{
   public:
    NVUINT32              base_addr;
    spec::AuxIndexType M_1;         // column M = M_1 + 1
    static const unsigned int width = 32 + spec::AuxIndexBits;
  ...
  Connections::Out<AuxMemType::aux_req_t> mem_wr_req;
  // Memory req/rsp for master write operation
  Connections::Out<AuxMemType::aux_req_t> mem_rd_req;
  Connections::In<AuxMemType::aux_rsp_t> mem_rd_rsp; 
  
  // Interconnect
  Connections::Combinational<WriteReqTrig> write_req_trig;
  
  SC_HAS_PROCESS(AuxAxi);
  AuxAxi(sc_module_name name_) : 
    sc_module(name_),
  ...
  void MasterWrite() {
    ...
    while(1) {
      ...
      while (num_words > 0) {
        ...
        w_req_trig_tmp.addr = (wr_addr_pld.len+1)*8*burst_count; // base addr of read req
      ...
      wr_IRQ.Push(1);
      CDCOUT(sc_time_stamp()  << name() << " Pushed AuxAxi Interrupt " << endl, 0);
      wait();
    }
  }
};
