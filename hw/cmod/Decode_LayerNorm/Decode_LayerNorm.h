...
#ifndef __DECODE_LAYERNORM_H__
#define __DECODE_LAYERNORM_H__
...
SC_MODULE(Decode_LayerNorm)
{
  ...
  SC_HAS_PROCESS(Decode_LayerNorm);
  Decode_LayerNorm(sc_module_name name_) : sc_module(name_) {
  ...
  void RunInputAddrGen() {
    ...
    while (1) { 
      ...
      if (basic_ready)  { 
         CDCOUT(sc_time_stamp()  << name() << " - DUT: Decode_LayerNorm mode" << endl, 0);
         NVUINT18 M_LN = 2*num_vector*num_timestep;
         ...
         for (int i = 0; i < M_LN; i++) {
           ...
           for (int i = 0; i < N; i++) {
             ...
             //CDCOUT(sc_time_stamp() << name() <<  " - DUT: Decode/LayerNorm input req address from decoder: " << req_reg.addr[i] << endl, 0);
           ...
           if (count == count_num) {
              ...
              k += 1;
              NVUINT12 k_reg = k;
              NVUINT1 lsb = nvhls::get_slc<1>(k_reg, 0);
