...
#ifndef __DECODE_N1_H__
#define __DECODE_N1_H__
...
SC_MODULE(Decode_N1)
{
  ...
  SC_HAS_PROCESS(Decode_N1);
  Decode_N1(sc_module_name name_) : sc_module(name_) {
  ...
  void RunInputAddrGen() {
    ...
    while (1) { 
      ...
      if (basic_ready)  { 
        ...
        CDCOUT(sc_time_stamp()  << name() << " - DUT: Decode: N1 mode" << endl, 0);
        ...
        for (int i = 0; i < M_DecN1; i++) {
          ...
          for (int i = 0; i < N; i++) {
            ...
            //cout << sc_time_stamp() << name() <<  " - DUT: Decode/InputAddrGen1 - input req address from decoder: " << req_reg.addr[i] << " with k: " << k << endl;
