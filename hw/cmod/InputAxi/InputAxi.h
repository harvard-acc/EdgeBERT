...
SC_MODULE(InputAxi)
{
  ...
  class WriteReqTrig: public nvhls_message{
   public:
    ...
    static const unsigned int width = spec::AddrType::width + 8;
 ...
 public: 
  ...
  typedef DecodeTop InputMemType;  
  ...
  Connections::Out<InputMemType::input_req_t> mem_wr_req;
  // Memory req/rsp for master write operation
  Connections::Out<InputMemType::input_req_t> mem_rd_req;
  Connections::In<InputMemType::input_rsp_t> mem_rd_rsp; 
  ...
  void MasterRead() {
    ...
    rd_IRQ.Reset();
    while(1) {
      InputMemType::input_req_t req_reg;
      ...
      while (num_words > 0) {
        ...
        rd_addr_pld.addr = base_addr + burst_count*8*(rd_addr_pld.len+1);
        ...
        for (int i = 0; i <= rd_addr_pld.len; i++) {
          ...
          for (int j = 0; j < N; j++) {
            if (j < 8) {
              ...
              req_reg.addr[j] = j + 8*i + burst_count*8*(rd_addr_pld.len+1);
              //cout << "InputAxi addr: " << req_reg.addr[j] << endl;
	            req_reg.valids[j] = 1;
	          }
	          else {
              req_reg.data[j] = 0; 
  ...
  void MasterWriteReq() {
    ...
    while(1) {
      ...
      spec::AddrType addr = w_req_trig_tmp.addr;    // Base addr of this burst
      NVUINT8               len = w_req_trig_tmp.len; // (len+1) 64-bit words
      
      //cout << "trig.addr: " << addr << "\t" << "trig.len: " << len << endl;
      //cout << "addr bit: " << spec::AddrType::width << endl;
      #pragma hls_pipeline_init_interval 1
      for (int i = 0; i <= len; i++) {
	      #pragma hls_unroll yes
          ...
          if (j < 8) {
            ...
            //cout << "InputAxi/MasterWriteReq addr: " << req_reg.addr[j] << endl;
            req_reg.valids[j] = 1;
	        }
	        else {
	          req_reg.addr[j] = 0;
    ...
    while(1) {
      ...
      if (valid == 1) {
        ...
        while (num_words > 0) {
          ...
          wr_addr_pld.addr = base_addr + burst_count*8*(wr_addr_pld.len+1);
          if_data_wr.aw.Push(wr_addr_pld);
          
          // 2. Push req trigger
          WriteReqTrig w_req_trig_tmp;
          w_req_trig_tmp.addr = (wr_addr_pld.len+1)*8*burst_count + base_output_reg; // base addr of read req
          ...
          InputMemType::input_rsp_t rsp_reg;
        ...
        wr_IRQ.Push(1);
        CDCOUT(sc_time_stamp()  << name() << " Pushed InputAxi Interrupt " << endl, 0);
      } //if valid == 1
      wait();
    }
  }
